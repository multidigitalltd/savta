...
- **ניגודיות (WCAG AA):** טקסט קרם על הוורוד `#C98573` נותן 2.8:1 בלבד, ולכן כפתורי ה-pill (ניווט, "אני רוצה שיחזרו אליי") וטקסטים קטנים בוורוד (מספרי סעיפים, eyebrow, קישור "איך זה עובד?") משתמשים ב-`#8F4531` (6.5:1) מפלטת העיצוב; מספרי השלבים הגדולים ב-`#BE7A66` (3.2:1, טקסט גדול). כדי לחזור לצבע המקורי בכפתורים: `--btn-rose: var(--rose)` ב-`:root`.
